 ...
 *
 * 2015-11-18 jarodlau
 * Changed to supply chinese usbasp...
 */
...
//#define ledRedOn()    (PORTD &= ~(1 << PD6), DDRD |= (1 << PD6))
//#define ledRedOff()   (PORTD |= (1 << PD6), DDRD &= ~(1 << PD6))
//#define ledGreenOn()  (PORTD &= ~(1 << PD5), DDRD |= (1 << PD5))
//#define ledGreenOff() (PORTD |= (1 << PD5), DDRD &= ~(1 << PD5))
/* 因为两个led是地对vcc连接
 * 所以为了保持绿色灯亮表示待命
 * 红色灯表示读取和存储(和平时习惯保持一致)
 */
#define ledRedOn()      (PORTD &= ~(1<<PD6),PORTD |= (1<<PD5))
#define ledRedOff()     (PORTD |= (1<<PD6),PORTD &= ~(1<<PD5))
#define ledGreenOn()    (PORTD &= ~(1<<PD5),PORTD |= (1<<PD6))
#define ledGreenOff()   (PORTD |= (1<<PD5),PORTD &= ~(1<<PD6))
